    ...
    def __init__(self):
        ...
        def notas():
            ...
            def editar(evento):
                ...
                self.botao_salvar_e.place(x=264, y=180)
                self.botao_deletar_e.place(x=230, y=220)
                self.botao_leitura_e.place(x=300, y=220)
        ...
        def config():
            ...
            def valida_hexa():
                ...
                if len(num_hexa) > 7:
                    ...
                    self.erro_alpha_gradiente = CTkLabel(self.frame_CORES, text="Erro: Não suportamos valores Alpha!", font=fonte_erros_inter)
                    self.label_voz.pack(pady=34)
                    self.erro_alpha_gradiente.place(x=60, y=225)
                    self.after(2500, tira_erro_alpha)
                elif len(num_hexa) < 7:
                    self.entrada_hexa.configure(border_color="red")
                    self.erro_tamanho = CTkLabel(self.frame_CORES, text="Erro: Valor pequeno demais!", font=fonte_erros_inter)
                    self.label_voz.pack(pady=34)
                    ...
                    self.entrada_hexa.configure(border_color="red")
                    self.erro_hash = CTkLabel(self.frame_CORES, text="Erro: Verifique Se Tem Hashtag No Início!", font=fonte_erros_inter)
                    self.label_voz.pack(pady=34)
